...
@router.get('/{documento}/elegibilidad',
...
async def verificar_elegibilidad(
    ...
    return ElegibilidadResponse(
        ...
        mensaje     = 'Activo y elegible para servicios' if es_elegible
...
async def listar_afiliados(
    ...
    result = await db.execute(
        text('SELECT id_afiliado, documento, nombre, apellido, regimen, estado, fecha_afil AS fecha_afiliacion FROM afiliado LIMIT :lim'),
        {'lim': min(limite, 100)}
    )
